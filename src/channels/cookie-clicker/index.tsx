...
const htmlBuildingIcons = new Image();
...
export function cookieClicker(props: ChannelProps) {
	...
	const [particles, setParticles] = useState<Array<EmotionJSX.Element>>([]);
	...
	return (
		<Container>
			...
			<VerticalSection style={{ overflow: 'clip' }}>
				...
				<FormattedText>
					<TweenNumber value={Math.floor(total?.raw ?? 0)} /> cookies
					<br />
			...
			<VerticalSection style={{ left: 'calc(30% + 16px)', width: '45%', bottom: '0' }}>
				<AnnouncementSection>
					<AnnouncementText
						css={css`
							animation: ${staticFadeUp} 5s;
							animation-iteration-count: 1;
						`}
						ref={announcementRef}
						onAnimationEnd={() => announcementTicker(setAnnouncementText)}>
						{announcementText}
					</AnnouncementText>
				</AnnouncementSection>
...
function announcementTicker(setText: Function) {
	const announcementIndex = random(0, flavorText.length - 1);
...
function drawBuilding(donation: FormattedDonation) {
	...
	for (let i = buildings.length - 1; i >= 0; i--) {
		...
		if (donation.rawAmount > building.price) {
			const context = building.canvasRef?.current?.getContext('2d');
			...
			context?.drawImage(
				...
				64,
				64,
				(building.total % 10) * random(53, 60),
				building.total % 2 == 0
					? random(70 - building.yRandomization, 70)
					: random(85 - building.yRandomization, 85),
				64,
				64,
			);

			building.savedCanvas = context?.getImageData(
				0,
				0,
				building.canvasRef!.current!.width,
				building.canvasRef!.current!.height,
			);
...
function drawParticle(donation: FormattedDonation, setParticles: Function) {
	const particleLocationX = random(15, 70);
	const particleLocationY = random(35, 65);

	sequentialKey += 1;

	setParticles((prevParticles: Array<EmotionJSX.Element>) => [
		...prevParticles,
		<div
			key={sequentialKey}
			onAnimationEnd={() => {
			...
			<CookieParticle
				css={css`
					animation: ${ParticleAnimation(particleLocationX, particleLocationY)} 2s ease-out;
			...
			<div
				css={css`
					${FloatText(particleLocationX)};
					animation: ${FadeUpAnimation(particleLocationY)} 1.5s linear;
				`}>
				+ {donation.amount}
			</div>
		</div>,
