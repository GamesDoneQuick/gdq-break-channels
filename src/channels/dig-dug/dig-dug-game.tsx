...
export class DigDugGame {
	...
	public showDonationAmount(amount: number) {
		...
		// Position lower on screen so it doesn't interfere with sprites
		text.anchor.set(0.5, 0.5);
		text.x = CHANNEL_WIDTH / 2;
		text.y = CHANNEL_HEIGHT * 0.85; // Lower portion of screen
	...
	public chainPopAllEnemies() {
		// Get all enemies currently on screen
		const enemiesToPop = [...this.enemies];

		if (enemiesToPop.length === 0) return;

		// Pop them one by one in sequence with a delay
		enemiesToPop.forEach((enemy, index) => {
			setTimeout(() => {
				// Make sure enemy still exists
				if (this.enemies.includes(enemy)) {
					// Instantly inflate and pop
					enemy.inflationProgress = 1;
					setTimeout(() => {
						if (this.enemies.includes(enemy)) {
							this.popEnemy(enemy);
						}
